...
int ConnectBT(unsigned char* Name){           //Connect ESP32 to a BT device
    ...
    char *ReservedBufferBisPointer;
    ReservedBufferBisPointer = &ReservedBufferBis;
    int recSizeBis;
    CountExcess = 0;
    while(Serial_Read(ReservedBufferBisPointer, 1024, recSizeBis) == 1){   //waiting for a response
        CountExcess++;
        if(CountExcess >= 30000000){                                //TimeOut
            break;
        }
    }
...
void OpenBTList(){                          //Create the window to connect to a BT device
    ...
    int iresult5;
    GetFKeyPtr(1120, &iresult5);
    FKey_Display(1, iresult5);      //F2
...
void MainMenu(){
    ...
    int iresult4;
    GetFKeyPtr(992, &iresult4);
    FKey_Display(4, iresult4);      //F5
...
void main(void) {
    ...
    while(1){
        ...
        if(window == 0){
            ...
            if (key == KEY_CTRL_F5) {       //Start serial comunication with the ESP32 at 9600 bps if F1 is pressed
                Serial_Write("&Rst&", 6);
            }
